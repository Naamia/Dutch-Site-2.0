...
function allClear() { //clears answer results (correct/incorrect)
 ...
 xt =  () => document.classList.toggle('marked', false);

    document.querySelectorAll('.yes').forEach(xt);
    document.querySelectorAll('.no').forEach(xt);
...
for (var i = 0; i < obj.length; i++) {
    ...
    if (obj[i].verb[5] == q) { 
      ...
      var no = document.getElementsByClassName('no');
        ...
        if (obj[i][0] == ans1) { 
            yes[0].classList.toggle('marked', true);
            no[0].classList.toggle('marked', false);
            score += 1;
        } else  
          no[0].classList.toggle('marked', true);
          yes[0].classList.toggle('marked', false);
          correct1.innerHTML = "The correct answer is " + "'" + obj[i][0] + "'";
        if (obj[i][1] == ans2) { 
          ...
          correct2.innerHTML = "The correct answer is " + "'" + obj[i][1] + "'";
        if (obj[i][2] == ans3) { 
          ...
          correct3.innerHTML = "The correct answer is " + "'" + obj[i][2] + "'";
        if (obj[i][3] == ans4) { 
          ...
          correct4.innerHTML = "The correct answer is " + "'" + obj[i][3] + "'";
        if (obj[i][4] == ans5) { 
          ...
          correct5.innerHTML = "The correct answer is " + "'" + obj[i][4] + "'";
        if (obj[i][5] == ans6) { 
          ...
          correct6.innerHTML = "The correct answer is " + "'" + obj[i][5] + "'";
    } 
    

    scoreBoard.innerText = score + " / " + obj[i].length;
    verbScore + score;
    total.innerText = "Total Running Score: " + verbScore;
} 
score = 0;
if (current == obj.length - 1) {
  scoreBoard.innerText = "Congratulations your total score is " + verbScore + " / " + (obj.length - 1) * obj[1].length; //first 'verb' is empty
}
